import { moduleForModel, test } from 'ember-qunit';

moduleForModel('i-i-s-1123-единицы-измер', 'Unit | Serializer | i-i-s-1123-единицы-измер', {
  // Specify the other units that are required for this test.
  needs: [
    'serializer:i-i-s-1123-единицы-измер',
    'service:syncer',
    'transform:file',
    'transform:decimal',
    'transform:guid',

    'transform:i-i-s-1123-сост-оплаты',
    'transform:i-i-s-1123-сост-отгрузки',

    'model:i-i-s-1123-единицы-измер',
    'model:i-i-s-1123-заказ-пост',
    'model:i-i-s-1123-контрагенты',
    'model:i-i-s-1123-номенклатура',
    'model:i-i-s-1123-организации',
    'model:i-i-s-1123-отчет-о-зак',
    'model:i-i-s-1123-план-закупок',
    'model:i-i-s-1123-план',
    'model:i-i-s-1123-склады',
    'model:i-i-s-1123-т-ч-заказ',
    'validator:ds-error',
    'validator:presence',
    'validator:number',
    'validator:date',
    'validator:belongs-to',
    'validator:has-many',
  ],
});

// Replace this with your real tests.
test('it serializes records', function(assert) {
  let record = this.subject();

  let serializedRecord = record.serialize();

  assert.ok(serializedRecord);
});
